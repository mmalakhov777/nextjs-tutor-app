...
echo "Running tool call types migration script..."
echo "Database URL: ${DB_URL:0:20}..." # Show only first 20 chars for security

psql "$DB_URL" -f scripts/add_tool_call_types_to_chat_sessions.sql

if [ $? -eq 0 ]; then
  echo "Tool call types migration completed successfully!"
else
