...
async function insertAllScenarios() {
  for (const scenario of scenarios) {
    await sql`
      INSERT INTO scenarios (id, title, description, icon, color)
      VALUES (${scenario.id}, ${scenario.title}, ${scenario.description}, ${typeof scenario.icon === 'string' ? scenario.icon : scenario.icon?.name || 'Unknown'}, ${scenario.color})
    ...
    console.log(`Inserted scenario: ${scenario.id}`);
  }
}
