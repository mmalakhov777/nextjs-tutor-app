...
export function ScenariosModal({ isOpen, onOpenChange, onSelectScenario }: ScenariosModalProps) {
  ...
  // Function to handle saving a scenario
  const handleSaveScenario = async (scenario: ScenarioData) => {
    setIsSaving(true);
    try {
      ...
      if (response.ok) {
        ...
        alert(`Scenario saved successfully with ID: ${result.id}`);
      } else {
  ...
  const renderScenarioCard = (scenario: ScenarioData) => {
    return (
      <div 
        key={scenario.id}
        className="transition-all duration-200 hover:bg-slate-50 h-full relative group"
        style={{
          display: 'flex',
          padding: '16px',
      ...
      >
        ...
        <button
          ...
          disabled={isSaving}
          className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200 p-2 rounded-lg bg-white shadow-md hover:bg-gray-50 border border-gray-200"
          title="Save scenario"
        >
        ...
        >
          <div className="flex flex-col">
            <h3 className="font-semibold text-slate-900 line-clamp-1 pr-12">{scenario.title}</h3>
          </div>
